...
DOMAIN=${DOMAIN:-"staging.your-domain.com"}
API_DOMAIN=${API_DOMAIN:-"api.staging.your-domain.com"}
...
helm upgrade --install itinerario-staging ../../helm \
  --namespace $NAMESPACE \
  --create-namespace \
  --values ../../helm/values-staging.yaml \
  --set image.tag=$GIT_TAG \
  --set services[0].image.tag=staging \
  --wait
